...
castRayMulti(DEVICEPTR(Vector4f) &pt_out, std::vector<DEVICEPTR(uchar) *> &entriesVisibleType,
             int x, int y, std::vector<const CONSTPTR(TVoxel) *> &voxelData_vec,
             std::vector</*const*/ CONSTPTR(typename TIndex::IndexData) *> &voxelIndex_vec,
             Matrix4f invM, Vector4f invProjParams, float oneOverVoxelSize, float mu,
             const CONSTPTR(Vector2f) &viewFrustum_minmax) {
    ...
    std::vector<typename TIndex::IndexCache> cache_vec;


    while (totalLength < totalLengthMax) {
        for (size_t i = 0; i < voxelData_vec.size(); ++i) {
            sceneIsBackground = i == 0; //0 is BG
            auto *voxelData = voxelData_vec.at(i);
            auto *voxelIndex = voxelIndex_vec.at(i);
            typename TIndex::IndexCache cache;
            cache_vec.push_back(cache);

//TODO segfault here due to none existing blockPos
            sdfValue = readFromSDF_float_uninterpolated(voxelData, voxelIndex, pt_result, vmIndex, cache);
...
//                std::cout << sdfValue << " " << sdfValue_final << std::endl;
        ...
        stepLength = MAX(sdfValue_final * stepScale, 1.0f);
        pt_result += stepLength * rayDirection;
        totalLength += stepLength;
    }
    ...
    if (sdfValue_final <= 0.0f) {
        ...
        pt_result += stepLength * rayDirection;
        auto *voxelData = voxelData_vec.at(index_with_min_sdf_value);
        auto *voxelIndex = voxelIndex_vec.at(index_with_min_sdf_value);
        sdfValue_final = readWithConfidenceFromSDF_float_interpolated(confidence, voxelData, voxelIndex, pt_result,
                                                                      vmIndex, cache_vec.at(index_with_min_sdf_value));
...
template<bool useSmoothing, bool flipNormals>
_CPU_AND_GPU_CODE_ inline void processPixelICP(DEVICEPTR(Vector4f) *pointsMap, DEVICEPTR(Vector4f) *normalsMap,
                                               const CONSTPTR(Vector4f) *pointsRay, const THREADPTR(Vector2i) &imgSize,
...
template<class TVoxel, class TIndex>
_CPU_AND_GPU_CODE_ inline void
processPixelConfidence(DEVICEPTR(Vector4u) &outRendering, const CONSTPTR(Vector4f) &point,
                       bool foundPoint, const CONSTPTR(TVoxel) *voxelData,
                       const CONSTPTR(typename TIndex::IndexData) *voxelIndex,
                       Vector3f lightSource) {
    Vector3f outNormal;
